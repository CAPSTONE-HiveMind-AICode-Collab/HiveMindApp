...
export async function GET(req, { params }) {
  ...
  if (!session) {
    return new Response("Sandbox terminal session not found.", { status: 404 });
  }
  ...
  if (String(authResult.auth?.uid || "") !== String(session.meta?.uid || "")) {
    return new Response("This terminal session belongs to another member.", { status: 403 });
  }
        ...
        try {
          ...
          if (!currentSession) {
            send("done", {
              success: false,
              exitCode: null,
              summary: "Sandbox terminal session could not be found while streaming output.",
              timestamp: Date.now(),
            });
            closeStream();
            return;
          ...
          send("done", {
            success: false,
            exitCode: null,
            summary: String(error?.message || "Sandbox terminal stream failed."),
            timestamp: Date.now(),
          });
          closeStream();
        } finally {
