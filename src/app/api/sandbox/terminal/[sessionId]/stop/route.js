...
import { stopSandboxTerminalSession } from "@/lib/server/sandbox/terminal";
...
export async function POST(req, { params }) {
  try {
    ...
    const result = await stopSandboxTerminalSession(sessionId, {
      reason: String(payload?.reason || "Terminal stopped.").trim() || "Terminal stopped.",
    });
    return NextResponse.json(result);
  } catch (error) {
    console.error("Sandbox terminal stop route failed:", error);
    return NextResponse.json(
      { error: String(error?.message || "Could not stop the sandbox terminal.") },
      { status: 500 }
    );
  }
}
